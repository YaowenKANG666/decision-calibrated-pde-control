...
1. **Method claim:** perturbation-normalized predictive scores define calibrated
   L2, ellipsoidal, and simultaneous-box dynamics sets; robust MPC subsequently
   queries their support in cost-to-go sensitivity directions.
2. **Theory claim:** the support function is exact for the stated diagonal
   ellipsoid and coordinate box; the conformal guarantee is marginal under
   exchangeability.
...
4. **Empirical claim:** in the final seed-27 matched actuator sweep, box-adjoint
   MPC reduces mean/p90 cost by 1.89%/10.84% versus nominal MPC. This is
   mechanism evidence from one controlled sweep, not a multi-seed population
   claim.
